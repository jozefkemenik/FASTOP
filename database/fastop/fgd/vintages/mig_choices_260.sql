...
DECLARE
    ...
    SELECT DISTINCT ATTR.*, 
     ...
     ;
     --TAKE ONLY CURRENT ENTRIES(ROUND_SID = 260)
                ...
                l_debug := 3;
                    BEGIN
                        IF l_attr_name like '%_SID' THEN
                            ...
                            VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, l_new_question_value, null,  0);
                        ELSE
                            INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                            VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, l_question_value, null,  0);
                        END IF;
                    EXCEPTION
                    WHEN OTHERS THEN
            ...
            ELSIF r_rec.ATTR_TABLE NOT IN ( 'FGD_NFR_RULE_ASSESSMENTS', 'FGD_NFR_RULES', 'FGD_NFR_RULE_RULES', 'FGD_NFR_RULE_SECTORS', 'FGD_NFR_TARGET_RULES') AND
                  r_rec.ATTR_TABLE NOT LIKE '%ASSESS%' THEN
                    ...
                    LOOP
                        ...
                        BEGIN
                            INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                            VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, get_lov_sid(t_result.ATTR_VALUE),  t_result.DESCR, 0);
            ...
            IF r_rec.ATTR_TABLE = 'FGD_IFI_INSTITUTIONS' THEN
                ...
                BEGIN
                    IF l_attr_name like '%_SID' OR l_attr_name = 'BASIS_EXAA' OR l_attr_name = 'BASIS_EXPA' or l_attr_name = 'LEGAL_STATUS' OR l_attr_name = 'TRIG_ESCC_MON' THEN 
                        INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                        VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, get_lov_sid(l_question_value), null,  0);
                    ELSE
                        INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                        VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, l_question_value, null,  0);
                    END IF;
            ...
            IF r_rec.ATTR_TABLE = 'FGD_MTBF_FRAMES' THEN
                ...
                BEGIN
                    ...
                    ELSIF l_attr_name like '%_SID' AND l_question_value IS NULL THEN
                        INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                        VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, NULL, NULL,  0);
                    ELSE
                        INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                        VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, l_question_value, null,  0);
                    END IF;
...
--        dbms_output.put_line(r_rec.ATTR_NAME || l_debug);
        l_attr_name := r_rec.ATTR_NAME;
            IF r_rec.ATTR_TABLE = 'FGD_NFR_RULE_ASSESSMENTS' AND r_entry.TAB = 'NFR' THEN
                ...
                --TAKE period_sid = 1 
                    l_statement := 'SELECT '|| r_rec.ATTR_NAME ||' FROM ' || r_rec.ATTR_TABLE || ' WHERE RULE_SID = ' || r_entry.OLD_SID ||
                        ' AND PERIOD_SID = 1 AND ROUND_SID = 260';
                    BEGIN
                        EXECUTE IMMEDIATE l_statement INTO l_question_value;
                    EXCEPTION
                        WHEN OTHERS THEN
                    ...
                    l_debug := 2;
                        ...
                        IF l_section_assessment_period = 1 THEN
                            IF l_attr_name NOT like '%_SID' THEN
                                l_debug := 2224;
                                INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                                VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, l_question_value, NULL,  1);
                            ELSIF l_attr_name like '%_SID' AND l_question_value IS NOT NULL THEN
                            l_debug := 2222;
                                INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                                VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, get_lov_sid(l_question_value), NULL, 1);
                            ELSIF l_attr_name like '%_SID' AND l_question_value IS NULL THEN
                            l_debug := 2223;
                                INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                                VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, NULL, NULL, 1);
                            END IF;
                        END IF;
                        EXCEPTION
                        WHEN OTHERS THEN
                ...
                IF l_assessment_exists IS NOT NULL AND r_rec.ATTR_NAME != 'COMPLIANCE_SOURCE_SID' THEN
                    l_statement := 'SELECT '|| r_rec.ATTR_NAME ||' FROM ' || r_rec.ATTR_TABLE || ' WHERE RULE_SID = ' || r_entry.OLD_SID ||
                        ' AND PERIOD_SID = 2 AND ROUND_SID = 260';
                    l_debug := 313;
                    BEGIN
                        EXECUTE IMMEDIATE l_statement INTO l_question_value;
                    EXCEPTION
                    ...
                    IF l_section_assessment_period = 1 THEN
                        IF l_attr_name NOT like '%_SID' THEN
                            l_debug := 3224;
                            INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                            VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, l_question_value, null,  2);
                        ELSIF l_attr_name like '%_SID' AND l_question_value IS NOT NULL THEN
                        l_debug := 3222;
                            INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                            VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, get_lov_sid(l_question_value), null,  2);
                        ELSIF l_attr_name like '%_SID' AND l_question_value IS NULL THEN
                        l_debug := 3223;
                            INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                            VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, NULL, NULL,  2);
                        END IF;
                    END IF;
                    EXCEPTION
                    WHEN OTHERS THEN
            ...
            ELSIF r_rec.ATTR_TABLE != 'FGD_NFR_RULE_ASSESSMENTS' AND r_entry.TAB = 'NFR' AND
                  r_rec.ATTR_TABLE LIKE '%NFR%' THEN
                    ...
                    IF l_assessment_exists IS NOT NULL THEN
                        l_statement := 'SELECT RULE_ASSESSMENT_SID FROM FGD_NFR_RULE_ASSESSMENTS WHERE PERIOD_SID = 1 AND ROUND_SID = 260 AND RULE_SID = ' || r_entry.OLD_SID ;
                        ...
                        LOOP
                            ...
                            BEGIN
                                INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                                VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, get_lov_sid(t_result.ATTR_VALUE),  t_result.DESCR, 1);
                    ...
                    --PERIOD_SID = 2
                    IF l_assessment_exists IS NOT NULL THEN
                        l_statement := 'SELECT RULE_ASSESSMENT_SID FROM FGD_NFR_RULE_ASSESSMENTS WHERE PERIOD_SID = 2 AND ROUND_SID = 260 AND RULE_SID = ' || r_entry.OLD_SID ;
                        EXECUTE IMMEDIATE l_statement INTO l_assessment_sid;
                        l_query_string := 'SELECT ' || r_rec.ATTR_NAME || ', DESCR FROM ' || r_rec.ATTR_TABLE || ' WHERE RULE_ASSESSMENT_SID = ' || l_assessment_sid ;
                        l_debug := 24;
                        OPEN c_mig_multi FOR l_query_string;
                        ...
                        LOOP
                            ...
                            BEGIN
                                INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
                                VALUES (260, r_entry.NEW_SID, r_rec.NEW_QUESTION_VERSION_SID, get_lov_sid(t_result.ATTR_VALUE), t_result.DESCR, 2);
            ...
            ELSIF r_rec.ATTR_TABLE != 'FGD_IFI_INST_ASSESSMENTS' AND r_entry.TAB = 'IFI' AND
                  r_rec.ATTR_TABLE LIKE '%IFI%_ASS%' THEN
                    ...
                    IF r_rec.ATTR_TABLE = 'FGD_IFI_INST_TIME_COMP_ASSESS' THEN
                        l_query_string := 'SELECT ' || r_rec.ATTR_NAME || ', DESCR FROM ' || r_rec.ATTR_TABLE || ' WHERE INSTITUTION_SID = ' || r_entry.OLD_SID ;
                    ELSE 
                        l_statement := 'SELECT INSTITUTION_ASSESSMENT_SID FROM FGD_IFI_INST_ASSESSMENTS WHERE PERIOD_SID = 1 AND ROUND_SID = 260 AND INSTITUTION_SID = ' || r_entry.OLD_SID ;
                        EXECUTE IMMEDIATE l_statement INTO l_assessment_sid;
            ...
            ELSIF r_rec.ATTR_TABLE != 'FGD_MTBF_FRAME_ASSESSMENTS' AND r_entry.TAB = 'MTBF' AND
                  r_rec.ATTR_TABLE LIKE '%MTBF%' THEN
                    --PERIOD_SID = 1
                    l_statement := 'SELECT FRAME_ASSESSMENT_SID FROM FGD_MTBF_FRAME_ASSESSMENTS WHERE PERIOD_SID = 1 AND ROUND_SID = 260 AND FRAME_SID = ' || r_entry.OLD_SID ;
                    EXECUTE IMMEDIATE l_statement INTO l_assessment_sid;
...
BEGIN
    FOR r_ass_entry IN c_compliance LOOP
        FOR r_question IN c_old LOOP
            INSERT INTO ENTRY_CHOICES_ARCH(ROUND_SID, ENTRY_SID, QUESTION_VERSION_SID, RESPONSE, DETAILS,  ASSESSMENT_PERIOD)
            VALUES  (260, r_ass_entry.NEW_SID, r_question.NEW_QUESTION_VERSION_SID, r_ass_entry.COMPLIANCE_SOURCE_SID,  r_ass_entry.ASSESS_VAL,  r_ass_entry.PERIOD_SID);
        END LOOP;
    END LOOP;
EXCEPTION
    WHEN OTHERS THEN
